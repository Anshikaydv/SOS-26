...
class RNN:
    ...
    def softmax(self, x):
        ...
        p = exponential / np.sum(exponential)
    ...
    def forward(self, X, a_prev):
        ...
        for i in range(len(self.X)):
            ...
            if(i>0):
                ...
                a[i] = np.tanh(np.matmul(self.Waa, a_prev) + np.matmul(self.Wax, x[i]) + self.ba) 
    ...
    def predict(self, start):
        ...
        while(idx != newline_character and counter < max_chars):
            ...
            idx = np.random.choice(self.vocab_size, p=y_pred.ravel())
            x = np.zeros((self.vocab_size, 1))
            x[idx] = 1
            idxes.append(idx)
            counter += 1
